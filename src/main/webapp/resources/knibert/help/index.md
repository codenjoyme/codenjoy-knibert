...
За съеденные яблочки игроку начисляются бонусные очки[(?)](#ask) в
размере длинны змейки на данный момент за каждое яблоко.
За смерть змейки - могут отниматься штрафные очки[(?)](#ask).
...
Итак, игрок [регистрируется на сервере](../../../register?gameName=knibert),
указывая свой email.
