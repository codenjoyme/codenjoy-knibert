package com.codenjoy.dojo.knibert.services.ai;
...
public class AISolverTest {
    ...
    private void assertWay(String boardString, Path path) {
        ...
        Direction direction = board.getHeroDirection();
        List<Point> stones = board.getStones();
        List<Point> hero = board.getHero();
        List<Point> walls = board.getWalls();

        assertWay(board, path, from, to, direction, hero, merge(stones, walls));
    }
...
//    }
    ...
    private void assertWay(Board board, Path expectedPath, Point from, Point to, Direction direction, List<Point> hero, List<Point> barriers) {
        List<Point> allBarriers = board.getBarriers();
        List<Direction> actualPath = new LinkedList<Direction>();
        System.out.println("Now Hero at: " + hero.toString());
        do {
            ...
            board.set(hero.get(0).getX(), hero.get(0).getY(), Element.TAIL_VERTICAL.ch());
            hero = board.getHero();

            allBarriers = merge(hero, barriers);
    ...
    private LinkedList<Point> merge(List<Point> hero, List<Point> barriers) {
        LinkedList<Point> allBarriers = new LinkedList<Point>();
        allBarriers.addAll(barriers);
        allBarriers.addAll(hero);
        return allBarriers;
    }
